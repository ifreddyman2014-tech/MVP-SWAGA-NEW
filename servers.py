...
class VPNServer:
    ...
    xui_ssl: bool = True             # Использовать HTTPS для API (False = HTTP)
...
class ServerManager:
    ...
    def load_config(self, config_path: str = SERVERS_CONFIG_PATH) -> bool:
        ...
        try:
            ...
            # Заполняем current_users из БД чтобы балансировка работала корректно
            try:
                ...
                rows = conn.execute(
                    ...
                    "WHERE is_active=1 AND end_date >= date('now') GROUP BY server_id"
                ).fetchall()
                conn.close()
                for server_id, count in rows:
                    if server_id in self.servers:
                        self.servers[server_id].current_users = count
                        logger.info("Сервер %s: %d активных пользователей", server_id, count)
            except Exception as e:
                logger.warning("Не удалось загрузить current_users из БД: %s", e)
                ...
                "servers": [
                    {
                        ...
                        "reality_spx": s.reality_spx,
                        "ws_ssh_password": s.ws_ssh_password,
                        "ws_config_path": s.ws_config_path,
                    }
    ...
    async def check_server_health(self, server: VPNServer) -> bool:
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout, cookie_jar=jar) as session:
                ...
                csrf_token = None
                try:
                    async with session.get(f"{base}/", ssl=False) as r:
                        html = await r.text()
                        import re as _re
                        m = _re.search(r'csrf-token" content="([^"]+)"', html)
                        if m:
                ...
                async with session.post(
                    f"{base}/login",
                    data={"username": server.xui_username, "password": server.xui_password},
                    headers=headers,
                    ssl=False,
                ) as resp:
                    try:
                    ...
                    is_healthy = bool(data.get("success", False))
