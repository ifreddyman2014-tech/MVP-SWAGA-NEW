...
def build_vless_link(
    ...
    """Сформировать VLESS ссылку для подключения (TCP, xHTTP или WS+TLS через Cloudflare)."""
    transport = (transport or "").strip() or "xhttp"
    enc_remark = quote(remark, safe="")

    if transport == "ws":
        # WS+TLS через Cloudflare CDN — без Reality, security=tls
        sni = camouflage_host or host
    ...
    # Базовая часть ссылки для Reality-транспортов
    ...
    if transport == "tcp":
        ...
        # xHTTP и прочие Reality-транспорты
        enc_path = quote(path, safe="")
        params = (
            f"&type={transport}"
            f"&path={enc_path}"
